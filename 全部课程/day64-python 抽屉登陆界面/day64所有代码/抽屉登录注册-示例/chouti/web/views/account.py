...
import json
import datetime
from backend.utils.message import email as sendemail
...
def send_msg(request):
    ...
    if form.is_valid():
        ...
        print("发送验证码开始时间：",current_date)
        code = commons.random_code()  #ATGJ
        sendemail(email,code)
        count = models.SendMsg.objects.filter(email=email).count()  #是否存在email
        if not count:
...
def register(request):
    ...
    if form.is_valid():
        # print("数据格式通过")
        current_date = datetime.datetime.now()
        limit_day = current_date - datetime.timedelta(minutes=5)
        print(limit_day,'limit_day')
        _value_dict = form.clean()
        print(_value_dict)
        ...
        print('is_valid_code',is_valid_code)
